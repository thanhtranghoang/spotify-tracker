...
# ─── GROUP 2 (KHÓC CÙNG EM) ──────────────────────────────────
YT_DATA_FILE_2 = "youtube_data_2.json"
VIDEO_IDS_2 = [
    "aXowvLRLkfM",
    "pIWH3cHu6Ms"
]
...
def load_data_2():
    if os.path.exists(YT_DATA_FILE_2):
        with open(YT_DATA_FILE_2) as f:
            return json.load(f)
    return {"last_run": "Never", "videos": {}}

def save_data_2(data):
    with open(YT_DATA_FILE_2, "w") as f:
        json.dump(data, f, indent=2)
...
if __name__ == "__main__":
    ...
    try:
        ...
        if not has_any_change(videos, prev_videos):
            print("⏭️ [Group 1] No change detected — skipping Discord message.")
        else:
            print("✅ [Group 1] Changes detected — sending to Discord...")
            send_to_discord(videos, data)
        data["last_run"] = format_time()
        data["videos"] = build_updated_videos(videos, prev_videos, now_iso)
        save_data(data)
        print("Group 1 data saved. ✅")

    # ── Group 2 (KHÓC CÙNG EM – 2 videos) ───────────────────
    data2 = load_data_2()
    prev_videos2 = data2.get("videos", {})
    try:
        videos2 = fetch_video_stats(VIDEO_IDS_2)
    except Exception as e:
        msg = f"[Group 2] YouTube API error: {e}"
        print(f"❌ {msg}")
        send_error_to_discord(msg)
    else:
        for v in videos2:
            print(f"  #{videos2.index(v)+1} {v['title'][:40]}: {v['views']:,} views")
        if not has_any_change(videos2, prev_videos2):
            print("⏭️ [Group 2] No change detected — skipping Discord message.")
        else:
            print("✅ [Group 2] Changes detected — sending to Discord...")
            send_to_discord(videos2, data2)
        data2["last_run"] = format_time()
        data2["videos"] = build_updated_videos(videos2, prev_videos2, now_iso)
        save_data_2(data2)
        print("Group 2 data saved. ✅")
